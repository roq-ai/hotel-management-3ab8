...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'booking.create.create': { roles: ['admin', 'hotel-manager'], key: 'bookingId', tenantPath: [], userPath: [] },
  'payment.receive.create': { roles: ['admin', 'hotel-manager'], key: 'paymentId', tenantPath: [], userPath: [] },
  'room.update.update': { roles: ['admin', 'hotel-manager'], key: 'roomId', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['admin'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['admin', 'hotel-manager'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.hotel.findFirst({
