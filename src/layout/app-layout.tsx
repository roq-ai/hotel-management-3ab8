...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['admin', 'hotel-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<HotelInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/hotels` : null),
    () => getHotels({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Hotels',
      path: isTenantUser ? `/hotels/view/${data?.[0]?.id}` : '/hotels',
      entity: 'hotel',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    { name: 'Bookings', path: '/bookings', entity: 'booking', service: AccessServiceEnum.PROJECT, icon: FiCalendar },
    { name: 'Profits', path: '/profits', entity: 'profit', service: AccessServiceEnum.PROJECT, icon: FiDollarSign },
